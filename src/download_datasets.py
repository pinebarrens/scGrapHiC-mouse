...
def download_hires_scrnaseq_datasets():
    ...
    if not os.path.exists(embryo_scrnaseq_data):
        wget.download(
...
# Standalone CLI to download the required datasets
if __name__ == '__main__':
    import argparse

    p = argparse.ArgumentParser(description='Download scGrapHiC reference / HiRES datasets.')
    p.add_argument(
        '--datasets', default='all',
        choices=['all', 'structure', 'schic', 'scrnaseq'],
        help="Which dataset group to fetch: 'structure' = dir layout + GTF + bulk "
             "Hi-C + metadata; 'schic'/'scrnaseq' = the HiRES raw data; 'all' = everything.",
    )
    args = p.parse_args()

    if args.datasets in ('all', 'structure'):
        create_directory_structure()
    if args.datasets in ('all', 'schic'):
        download_hires_schic_datasets()
    if args.datasets in ('all', 'scrnaseq'):
