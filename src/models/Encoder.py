...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, dim_q, dim_k, dim_v, num_heads=8, dropout_prob=0.1, dim_a=None, dim_o=None, use_alibi=False):
        ...
        if self.use_alibi:
            ...
            self.log_slopes = nn.Parameter(log_slopes.view(self.num_heads, 1, 1))
    ...
    def forward(self, q, k, v, mask=None):
        ...
        a = q @ k.transpose(-1, -2) / (self.dim_a // self.num_heads) ** 0.5
        if self.use_alibi:
        ...
        o = (a @ v).transpose(1, 2).flatten(2, 3)
        o = self.fc_o(o)
        return o
...
class GraphConvolution(pl.LightningModule):
    ...
    def forward(self, x, edge_index, edge_attr, batch):
        x_ = self.ln(x)
        x_ = self.conv(x_, edge_index, edge_attr)
