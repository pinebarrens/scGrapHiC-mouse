...
def pseudobulk_stem(path):
    # Parse filenames with dots in cell types
    name = path.replace("\\", "/").split("/")[-1]
    for suffix in (".csv.gz", ".csv", ".pairs.gz", ".pairs"):
...
def get_file_name_parameters(path):
    parameters = pseudobulk_stem(path).split("_")
    idx = 0
    stage = None
    if len(parameters) > 4:
...
def initialize_parameters_from_args():
    ...
    # Inference / runtime I/O
    parser.add_argument('--checkpoint', type=str, default=None, help='Path to a model .ckpt to load for inference')
