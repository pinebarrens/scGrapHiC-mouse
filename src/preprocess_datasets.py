...
def read_cell_by_gene_matrix(path):
    ...
    if name.endswith(".csv.gz") or name.endswith(".csv"):
        sep = ","
    elif name.endswith(".tsv.gz") or name.endswith(".tsv") or name.endswith(".txt"):
        sep = "\t"
    else:
        sep = None
...
def create_coordinate_matrix_file(scrna_seq_file, gene_cooridate_file_path, output_folder, PARAMETERS):
    scrna_seq_file_name = pseudobulk_stem(scrna_seq_file)
...
def create_genomic_track_file(preprocessed_coordinate_matrix, PARAMETERS, output_path, pseudobulk=False, chrom_sizes_path=None):
    chrom_sizes = read_chromsizes_file(chrom_sizes_path or CHROM_SIZES_FILE)
...
def parse_hires_scrnaseq_datasets(input_path, PARAMETERS, output_path, pseudobulk=False,
                                  gtf_path=None, chrom_sizes_path=None, gene_coordinate_file=None):
    '''
        This function parses the scRNA-seq datasets from cell-by-gene to geneome coordinate tracks.
    '''

    gtf_path = gtf_path or MM10_GTF3_FILE_PATH
    chrom_sizes_path = chrom_sizes_path or CHROM_SIZES_FILE
    ...
    if gene_coordinate_file is None:
        gene_coordinate_file = os.path.join(output_path, 'gene_coordinates.csv')
    gene_cooridate_file_path = gene_coordinate_file
...
def parse_hires_schic_datasets(input_path, PARAMETERS, output_path, chrom_sizes_path=None):
    create_directory(output_path)
    chrom_sizes = read_chromsizes_file(chrom_sizes_path or CHROM_SIZES_FILE)
    ...
    for schic_file in schic_files:
        if '.pairs' in schic_file and os.path.exists(schic_file):
            ...
            cell_name = pseudobulk_stem(schic_file)
...
# Standalone CLI to run a single preprocessing step against arbitrary input/output paths

if __name__ == '__main__':
    import argparse

    parser = argparse.ArgumentParser(
        description='Preprocess scRNA-seq / scHi-C inputs into genomic tracks / contact matrices.'
    )
    sub = parser.add_subparsers(dest='command', required=True)

    p_rna = sub.add_parser('scrnaseq', help='Parse a cell-by-gene matrix (file or directory) into genomic tracks.')
    p_rna.add_argument('--matrix', required=True, help='Cell-by-gene matrix file, or a directory of such files.')
    p_rna.add_argument('--output_dir', required=True)
    p_rna.add_argument('--gtf', default=None, help='GTF/GFF3 annotation (default: globals / SCGRAPHIC_GTF).')
    p_rna.add_argument('--chrom_sizes', default=None, help='Chromosome sizes file (default: globals / SCGRAPHIC_CHROM_SIZES).')
    p_rna.add_argument('--resolution', type=int, default=50000)
    p_rna.add_argument('--library_size', type=float, default=25000)
    p_rna.add_argument('--normalize_umi', action='store_true', default=False)
    p_rna.add_argument('--normalize_track', action='store_true', default=True)
    p_rna.add_argument('--pseudobulk', action='store_true', default=False,
                       help='Name outputs by the stage_tissue_celltype_nN convention.')

    p_hic = sub.add_parser('schic', help='Parse .pairs files into per-chromosome contact matrices.')
    p_hic.add_argument('--pairs_dir', required=True, help='Directory of .pairs files.')
    p_hic.add_argument('--output_dir', required=True)
    p_hic.add_argument('--chrom_sizes', default=None, help='Chromosome sizes file (default: globals / SCGRAPHIC_CHROM_SIZES).')
    p_hic.add_argument('--resolution', type=int, default=50000)

    args = parser.parse_args()

    if args.command == 'scrnaseq':
        PARAMETERS = {
            'resolution': args.resolution,
            'library_size': args.library_size,
            'normalize_umi': args.normalize_umi,
            'normalize_track': args.normalize_track,
        }
        parse_hires_scrnaseq_datasets(
            args.matrix, PARAMETERS, args.output_dir,
            pseudobulk=args.pseudobulk,
            gtf_path=args.gtf,
            chrom_sizes_path=args.chrom_sizes,
        )
    elif args.command == 'schic':
        PARAMETERS = {'resolution': args.resolution}
        parse_hires_schic_datasets(
            args.pairs_dir, PARAMETERS, args.output_dir,
            chrom_sizes_path=args.chrom_sizes,
        )
