...
# Path-flexible inputs
npz_path = PARAMETERS['npz'] or os.path.join(MOUSE_PROCESSED_DATA_HIRES, 'test.npz')
results_dir = PARAMETERS['results'] or RESULTS
ckpt_path = PARAMETERS['checkpoint'] or os.path.join(MODEL_WEIGHTS, 'scgraphic')
device = PARAMETERS['device'] or 'cpu'

test_dataset = GenomicDataset(
    npz_path,  # was os.path.join(MOUSE_PROCESSED_DATA_HIRES, 'test.npz')
    PARAMETERS
)
...
evaluate(os.path.join(results_dir, PARAMETERS['experiment']), PARAMETERS)  # was RESULTS
