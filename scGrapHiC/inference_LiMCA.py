...
MODEL_WEIGHTS = os.path.join("/users/mliu237/scratch", 'weights')
RESULTS = os.path.join("/users/mliu237/scratch/LiMCA", 'results')
...
test_dataset = GenomicDataset(
    os.path.join("/users/mliu237/scratch/LiMCA/processed", 'scgraphic_test.npz'),
    PARAMETERS
)
...
ckpt_path = os.path.join(MODEL_WEIGHTS, 'scgraphic.ckpt')
...
evaluate(os.path.join("/users/mliu237/scratch/LiMCA/results/scgraphic", PARAMETERS['experiment']), PARAMETERS)
# python /users/mliu237/scratch/scGrapHiC/inference_LiMCA.py --experiment scgraphic --rna_seq True --ctcf_motif True --cpg_motif True --use_bulk True --positional_encodings True --pos_encodings_dim 16
